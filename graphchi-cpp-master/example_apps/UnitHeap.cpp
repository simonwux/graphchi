...
void UnitHeap::ReConstruct()
{
	...
	while (!st.empty()){
		...
		int l = st.top();
		st.pop();
		int m = LinkedList[tmp[(l+r)/2]].key;
		int i = l;
		int j = r;
		while (i<=j) {
			while (LinkedList[tmp[i]].key>m) i++;
			while (LinkedList[tmp[j]].key<m) j--;
			if (i<=j){
				int t = tmp[i];
				tmp[i] = tmp[j];
				tmp[j] = t;
		...
		if (l<j) {st.push(l); st.push(j);}
		if (i<r) {st.push(i); st.push(r);}
	}
	//stack<int>().swap(st);
	// for (int i=0;i<tmp.size();i++){
	// 	std::cout<<"tmp"<<tmp[i]<<"key"<<LinkedList[tmp[i]].key<<"\n";
	// }
	//qsort(tmp.begin(),tmp.end(),tmp,LinkedList);
	//std::sort(tmp.begin(), tmp.end(), compare2);
	// for (int i=0;i<tmp.size()-1;i++){
	// 	for (int j=i+1;j<tmp.size();j++){
	// 		if (LinkedList[tmp[i]].key < LinkedList[tmp[j]].key){
	// 			int t = tmp[i];
	// 			tmp[i] = tmp[j];
	// 			tmp[j] = t;
	// 		}
